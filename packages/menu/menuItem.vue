<template>
  <div class="mzl-menu-div-box" :style="{'width':index!=0?'100%':''}">
    <li @mouseover="toggler(items)" @mouseleave="hide(items)" :data-key="dataKey" @click.stop="handelClick(items)" :style="itemLiStyle" :class="{'activeMenuItem':items.isActive,'menitemDisabled':items.disabled}">
      <div class="mzl-menu-item-title-box">
        ...
        <span class="menu-title" :class="spanClass" :style="activeColots">{{items.title}}</span>
        <span class="menu-icon-arrow" v-if="items.children&&items.children.length" :class="spanClass" :style="activeColots">
          <i :class="iconClass"></i>
        </span>
      </div>
       <!-- v-show="items.isOpen" v-if="items.children&&items.children.length" -->
       <transition name="slide-fade">
         <div class="mzl-menu-item-position-box" :style="mentItemStyle" v-show="items.isOpen" v-if="items.children&&items.children.length">
           <template v-for="(item,i) in items.children" :key="i">
             <menu-item :items="item" :index="index+(i+1)" :option="option" :data-key="dataKey+'-'+i" @change="emit('change',$event)" :theme="theme" :type="type"></menu-item>
           </template>
         </div>
       </transition>
    </li>
...
const themes = ref({
  bgColor:typeof(props.theme)=='string'?(props.theme=='black'?'#20222a':'#fff'):props.theme.bgColor,
  activeBgColor:typeof(props.theme)=='string'?(props.theme=='black'?'#000':'#eaeaea'):props.theme.activeBgColor,
  color:typeof(props.theme)=='string'?(props.theme=='black'?'#fff':'#333'):props.theme.color,
  activeColor:props.theme.activeColor,
})
const meunIconRotate = ref(false)
const tabkey = ref('')
const mentItemStyle = computed(()=>{
  return {'left':props.index===0?'0':'242px','top':props.index==0?'61px':'0px'}
})
const itemLiStyle = computed(()=>{
  return {
    'height':props.index==0?'58px':'40px',
    'line-height':props.index==0?'58px':'40px',
    'padding':props.index==0?'0 20px':(!props.items.icon?'0 10px 0 5px':'0 10px 0 10px')
  }
})
const iconClass = computed(()=>{
  return {
    'm-icon-arrow-down-bold':props.index==0,'m-icon-arrow-right':props.index!=0,
...
const handelClick = (item) =>{
  if(!item.disabled){
    ...
    getParents(props.option,item.key)[getParents(props.option,item.key).length-1].isOpen = false
    emit('change',item)
  }
}
</script>
